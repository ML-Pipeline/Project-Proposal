...
```{r}
bitcoins <- bitcoins %>% mutate(group_no = as.integer(factor(YearMonth)))

library(survival)
bitcoins$Status <- ifelse(bitcoins$PriceRatio>1,1,0)
#Survival Analysis
model= Surv(as.numeric(bitcoins$group_no), bitcoins$Status)
library(dplyr)
survfit(model~ 1, data=bitcoins) %>% plot(xlab="Years", ylab="p(survive)")
survfit(model~ 1, data=bitcoins) %>% summary()
plot(survfit(Surv(as.numeric(group_no), Status) ~ 1, data = bitcoins), 
     xlab = "Years", 
     ylab = "Overall survival probability")
# real data
ldat = bitcoins %>% tbl_df()
ldat
km = Surv(as.numeric(ldat$group_no), ldat$Status==1) %>%
  (function(x) survfit(x ~ 1, data=ldat))(.)
# Kaplan-Meier curve
plot(km, xlab= "Month",ylab="P(survive)")
km
```
```{r}
### Cox model
...
library(glmnet)
lm = ldat %>% na.omit() %>%
  (function(x) glmnet(x %>% select(-Month,-Status) %>% as.data.frame() %>% as.matrix(),
                      Surv(as.numeric(x$Month), x$Status==1),
                      family="cox"))(.)
coef(lm, s=0.1)

#How to choose s? Consider sparsity you want with cross-validation
cvlm = ldat %>% na.omit() %>%
  (function(x) cv.glmnet(x %>% select(-Month,-Status) %>%
                           as.data.frame() %>% as.matrix(),
                         Surv(as.numeric(x$Month), x$Status==1),
                         family="cox"))(.)
plot(cvlm)
plot(survfit(Surv(ldat$Month, ldat$Status==1) ~ PriceRatio>0.9, data=ldat),
...
```{r}
# Fit Cox Model
Cox_model = coxph(Surv(as.numeric(bitcoins$group_no),bitcoins$Status==1) ~Volatility+Competition+Volume..BTC.,data=bitcoins)
summary(Cox_model)
```
```{r}
...
```{r}
library(survminer)
bitcoins$Volatility <- bitcoins$High - bitcoins$Low
res.cox <- coxph(Surv(as.numeric(group_no), Status) ~ Volume..BTC.+Competition+Volatility, data = bitcoins)
summary(res.cox)

ggsurvplot(survfit(res.cox, data = bitcoins), color = "#2E9FDF",
